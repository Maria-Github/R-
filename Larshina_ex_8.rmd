...
# Модель 1 регрессионные деревья
...
p <- ggpairs(Carseats[, c(1, 6:8)])
suppressMessages(print(p))

p <- ggpairs(Carseats[, c(1, 9:11)])
suppressMessages(print(p))
```
...
Построим дерево регрессии для зависимой переменной Sales: миль на галлон.

```{r}
# Обучаем модель
tree.auto <- tree(Sales ~ ., Carseats, subset = train)
summary(tree.auto)
...
```{r}
# Прогноз по модели 
yhat <- predict(tree.auto, newdata = Carseats[-train, ])
auto.test <- Carseats[-train, "Sales"]
...
```
В данном случаем минимум ошибки соответствует самому сложному дереву, с 6 узлами. Покажем, как при желании можно обрезать дерево до 4 узлов (ошибка ненамного выше, чем минимальная).

# Модель 2 Обрезка деревьев
...
# матричные графики разброса переменных
p <- ggpairs(Carseats[, c(12, 1:4)], aes(color = high.Sales))
suppressMessages(print(p))

p <- ggpairs(Carseats[, c(12, 5:8)], aes(color = high.Sales))
suppressMessages(print(p))

p <- ggpairs(Carseats[, c(12, 9:11)], aes(color = high.Sales))
suppressMessages(print(p))
```

```{r}
# модель бинарного  дерева без переменных Sales и name
tree.auto <- tree(high.Sales ~ .-Sales, Carseats)
summary(tree.auto)
```
...
# тестовая выборка
auto.test <- Carseats[-train,]
high.Sales.test <- high.Sales[-train]

# строим дерево на обучающей выборке
tree.auto <- tree(high.Sales ~ .-Sales, Carseats, subset = train)
summary(tree.auto)
```
...
Обобщённая характеристика точности: доля верных прогнозов: 0,82
...
names(acc.test)[length(acc.test)] <- 'Carseats.class.tree.6'
acc.test
```

Точность этой модели практически не изменилась и составляет 0,79.
